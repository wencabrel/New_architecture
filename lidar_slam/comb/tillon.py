...
def visualize_final_result(file_path, max_entries=200, grid_resolution=0.05, 
                          save_grid=True, save_format='png', use_corner_preservation=True):
    """
    Process all LiDAR data and visualize only the final result without animation
    ...
    print(f"  Processing all data with grid resolution: {grid_resolution}m...")
    ...
    # Process all data frames to build the occupancy grid
    print("Processing LiDAR data to build occupancy grid...")
    for i, data in enumerate(parsed_data_list):
        # Progress indicator
        if i % 10 == 0:
            print(f"  Processing frame {i+1}/{len(parsed_data_list)}...")
        ...
        if use_corner_preservation:
            # Call the vectorized update directly if there are no points
            if len(x_points) == 0:
                continue  # Skip this frame if no points
                
            try:
                occupancy_grid.update_grid_with_corner_preservation(
                    robot_x, robot_y, theta, x_points, y_points, data['scan_ranges']
                )
            except ValueError:  # Handle the empty array case
                print(f"  Warning: Frame {i+1} produced no valid points, skipping...")
                continue
        else:
    ...
    # Display the occupancy grid on the right subplot
    grid_img = ax2.imshow(
        occupancy_grid.get_grid_for_display(enhance=True, show_dynamic=True, show_corners=use_corner_preservation),
        cmap=cmap, norm=norm,
        origin='lower',
    ...
    # Plot the robot path on the grid
    ax2.plot(robot_path_x, robot_path_y, 'b-', linewidth=2, label='Robot Path')
    
    # Plot start and end positions
    if len(robot_path_x) > 0:
        ax2.scatter(robot_path_x[0], robot_path_y[0], c='green', s=100, marker='*', label='Start')
        ax2.scatter(robot_path_x[-1], robot_path_y[-1], c='red', s=100, marker='*', label='End')
    
    # Plot detected corners if available
    if use_corner_preservation and hasattr(occupancy_grid, 'corner_tracker') and occupancy_grid.corner_tracker.corners:
        corner_x = [c['x'] for c in occupancy_grid.corner_tracker.corners]
        corner_y = [c['y'] for c in occupancy_grid.corner_tracker.corners]
        confidence = [c['confidence'] for c in occupancy_grid.corner_tracker.corners]
        sizes = [max(30, c['observations'] * 10) for c in occupancy_grid.corner_tracker.corners]
        
        ax2.scatter(corner_x, corner_y, c=confidence, s=sizes, 
                  cmap='viridis', marker='D', edgecolors='black', label='Corners')
    ...
    # Create a legend for the grid types
    legend_patches = []
    legend_labels = []
    ...
    # Add the legend
    ax2.legend(legend_patches, legend_labels, 
             loc='lower right', title='Map Legend',
             fontsize=8)
    
    # Add grid statistics as text
    stats_text = (f"Grid Statistics:\n"
                 f"Resolution: {grid_resolution:.3f}m/cell\n"
                 f"Grid Size: {occupancy_grid.grid_width}×{occupancy_grid.grid_height} cells\n"
                 f"Dimensions: {occupancy_grid.width:.1f}m × {occupancy_grid.height:.1f}m\n"
                 f"Occupied: {occupancy_grid.stats['occupied_cell_count']} cells\n"
                 f"Free: {occupancy_grid.stats['free_cell_count']} cells\n"
                 f"Unknown: {occupancy_grid.stats['unknown_cell_count']} cells\n"
                 f"Updates: {occupancy_grid.stats['updates']}, Resizes: {occupancy_grid.stats['resizes']}")
    
    # Add this text to the plot
    plt.figtext(0.02, 0.02, stats_text, wrap=True, fontsize=8, 
               bbox=dict(facecolor='white', alpha=0.7))
    
    # On the left subplot, display the final LiDAR scan
    # Get the last LiDAR scan
    last_data = parsed_data_list[-1]
    last_x_points, last_y_points = convert_scans_to_cartesian(
        last_data['scan_ranges'], angle_min, angle_max, last_data['pose'],
        flip_x=flip_x, flip_y=flip_y, reverse_scan=reverse_scan, flip_theta=flip_theta
    )
    
    # Plot the last scan
    ax1.scatter(last_x_points, last_y_points, c='blue', s=3, label='LiDAR Points')
    
    # Plot the robot position
    ax1.scatter(robot_path_x[-1], robot_path_y[-1], c='red', s=100, marker='*', label='Robot Position')
    
    # Plot the robot path
    ax1.plot(robot_path_x, robot_path_y, 'g-', linewidth=2, label='Robot Path')
    
    # Add robot orientation arrow
    theta = last_data['pose']['theta']
    if flip_theta:
        theta = -theta
    ...
    ax1.arrow(robot_path_x[-1], robot_path_y[-1], dx, dy, 
             head_width=0.1, head_length=0.1, fc='red', ec='red')
    
    # Set LiDAR scan plot properties
    ...
    ax1.set_title('Final 2D LiDAR Scan and Robot Path')
    ...
    config_text = (f"Configuration:\n"
                  f"  flip_x={flip_x}\n"
                  f"  flip_y={flip_y}\n"
                  f"  reverse_scan={reverse_scan}\n"
                  f"  flip_theta={flip_theta}\n"
                  f"  corner_preservation={use_corner_preservation}")
    ...
    save_button_ax = plt.axes([0.45, 0.05, 0.1, 0.04])
    ...
    # If save_grid is enabled, automatically save the final map
    if save_grid:
        # Create the save directory if it doesn't exist
        maps_dir = "maps"
        if not os.path.exists(maps_dir):
...
if __name__ == "__main__":
    ...
    file_path = "../dataset/raw_data/laser_data_synchronized_short_u_turn_fast_processed_reduced180.clf"
    
    # Run the static visualization instead of animation
    visualize_final_result(
        file_path, 
        max_entries=1800,          # Process up to 200 LiDAR scans
        grid_resolution=0.05,     # Grid resolution in meters (5cm per cell)
        save_grid=True,           # Save the final occupancy grid map
        save_format='png',        # Save as PNG
